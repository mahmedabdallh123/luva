...
# -------------------------------
# دوال OCR المحسنة (معالجة متقدمة للصورة)
# -------------------------------
def preprocess_image_for_ocr(image_bytes):
    """
    معالجة متقدمة للصورة لتحسين OCR:
    - تحويل إلى تدرج رمادي.
    - استخدام عتبة تكيفية (Adaptive Thresholding) للتعامل مع الإضاءة غير المنتظمة.
    - إزالة التشويش (Noise Removal).
    - تكبير الصورة لتحسين النص الصغير.
    """
    ...
    # 1. التحويل إلى التدرج الرمادي
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    
    # 2. تكبير الصورة (Rescaling) لتحسين وضوح النص
    (h, w) = gray.shape
    scaled = cv2.resize(gray, (w*2, h*2), interpolation=cv2.INTER_CUBIC)
    
    # 3. تقليل التشويش (Noise Removal)
    denoised = cv2.medianBlur(scaled, 3)

    # 4. العتبة التكيفية (Adaptive Thresholding)
...
def extract_raw_text_from_image(image_bytes):
    """
    استخراج النص الخام من الصورة بعد المعالجة.
    """
    processed = preprocess_image_for_ocr(image_bytes)
    if processed is None:
        return ""
    config = '--psm 6 -c preserve_interword_spaces=1'  # 6 = كتلة نصية واحدة
...
# تبويب استخراج الجدول من الصورة (مع تحسينات المعالجة)
if perms["can_input"] and OCR_AVAILABLE and "📸 استخراج جدول من صورة" in tabs_list:
    ...
    with tabs[idx]:
        ...
        st.markdown("""
        **كيف يعمل؟**
        - يستخرج النص من الصورة بعد معالجتها (تكبير، إزالة ضوضاء، عتبة تكيفية).
        ...
        if uploaded:
            ...
            if extracted_rows:
                ...
                # عرض النص الخام للمستخدم
                with st.expander("النص الخام المستخرج (يمكنك نسخه للمراجعة)"):
                    st.text(raw_text)
